...
{
    public class BasicCompletesState<T> : ICompletes<T>
    {
        protected readonly IActiveState<T> state;

        public BasicCompletesState(Scheduler scheduler) : this(new BasicActiveState<T>(scheduler))
        {
        }

        public BasicCompletesState(T outcome, bool succeeded) : this(new BasicActiveState<T>(), outcome, succeeded)
        {
        }

        public BasicCompletesState(T outcome) : this(new BasicActiveState<T>(), outcome)
        {
        }

        protected internal BasicCompletesState(IActiveState<T> state)
        {
            this.state = state;
        }

        protected internal BasicCompletesState(IActiveState<T> state, T outcome, bool succeeded)
        {
            this.state = state;
            if (succeeded)
            {
        ...
        protected internal BasicCompletesState(IActiveState<T> state, T outcome)
        {
            this.state = state;
        ...
        {
            var nestedCompletes = new BasicCompletesState<TO>(state.Scheduler);
            nestedCompletes.state.FailedValue(failedOutcomeValue);
            nestedCompletes.state.FailureAction((BasicCompletesState<TO>.Action<TO>)(object)state.FailureActionFunction());
            state.RegisterWithExecution((Action<T>)(object)BasicCompletesState<TO>.Action<TO>.With(function, nestedCompletes), timeout, state);
        ...
        {
            if (!state.HandleFailure(Optional.Of(outcome)))
            {
                state.CompletedWith(outcome);
            }
